...
Prior work has demonstrated the sensitivity of urban analysis outcomes to zone system design, from the way cities are visualized to the [impact of the nature of 'traffic analysis zones' on transport model outputs](http://www.iasi.cnr.it/ewgt/13conference/145_binetti.pdf). 
...
There is demand for visualisations techniques to represent new urban datasets within and cities, with the shift away from single-city case studies towards multi-city comparison research [@boeing_spatial_2021], between cities.
...
The approach we present in this paper is more closely analogous to 'grid sample' approaches used in ecological and population research [@hirzel_which_2002].
...
Limitations associated with this simplistic strategy have been documented since at least the 1960s.
A prominent paper on geographic sampling strategies outlining advantages and disadvantages of simple random, systematic and stratified sampling techniques in 1967, for example, found that the common confusion between 'area sampling' (in which entire areas from a more-or-less arbitrary zoning system or 'quadrats' are included or excluded) and 'location sampling' causes hidden biases in the selection of samples aiming to be representative of geographic phenomena [@holmes_problems_1967].
Starting with data at the level of raster grid cells and BSUs, a related approach is to sample from within available 'pixels' to generate a representative sample [@thomson_gridsample_2017].
...
This shows how it could be used to communicate key locations, including a train station (zone A) a park (zone C01) and the center of a neighboring city (zone E09), with reference to the city centre of Leeds, UK.
...
However, the example demonstrates how the system communicates potentially useful location data with a small amount of verbal information (3 characters).
...
```{r popdens, fig.cap="ClockBoard for 36 cities. The blue raster grid cells represent open access population estimates from the WorldPop project.", out.width="100%"}
...
Based on these criteria the ClockBoard zoning systems was developed, which consists of regularly spaced rings forming 'dohnuts' (technically annuli) emanating from the city centre and labelled A, B, C etc.
At the centre of the ClockBoard zoning system lies zone A, a circle measuring 1 km in radius.
...
The ClockBoard zoning system is a specific implementation of an approach to zone creation that we label 'concentric segmented annuli'.

A wide family of zoning systems could be created based on concentric segmented annuli.
Variations can be obtained by adjusting the sequence of outer ring radii (so they have values other than 1, 3, 6, 10, 15 and 21 km, resulting from the triangular number sequence used in the ClockBoard system) and number of segments (with values other than 1 for the central annuli and 12 for all others).
We implemented the concentric segmented annuli approach to zone generation in open source code to ensure reproducibility.
To encourage use of and adaptation of the system, we have packaged the code in the R package `zonebuilder` and in the Rust crate `zonebuilder`, which can be installed from the official 'CRAN' and 'crates.io' repositories, respectively.
...
In terms of the limitations of the *ClockBoard implementation of the concentric segmented annuli approach*, it is worth revisiting how we arrived at the ClockBoard system presented in this paper:
it is the result of an organic process of trial and error that included testing a range of alternative parameters that generated zones based on concentric segmented annuli.
Different approaches were discussed informally with others, including in a hackathon attended by around a dozen self selected people.
The ClockBoard system was found to meet many of the criteria outlined in Section \@ref(clockzs), providing a balance between the need for small zones (especially near city centers for many urban phenomena) and simplicity.
In other words, the zoning system is a compromise, with simplicity and 'readability' perhaps being the most important criteria in its design: the ability to describe any zone in a city with only 3 characters and a simple mental model based on the widely understood concept of a clockface was a key reason why the 12 segment system was chosen;
likewise, the triangular number sequence was selected for annuli radii because it resulted in a manageable number of zones even for large cities (ClockBoard zones composed of 10 concentric circles reach 55 km from the central point, reaching outer London, yet are composed of only 109 zones).

We do not believe that ClockBoard is in any way an 'optimal' zoning system: it generated subjectively better results when used to visualise datasets that we were using and interested in, such as the air pollution dataset presented in Figure \@ref(fig:cityscale), than when using administrative zones or other zoning systems based on concentric segmented annuli.
We acknowledge that other input parameters, and indeed other zoning system designs altogether, could have generated better results.
A specific (and deliberate) limitation of the ClockBoard zoning system is that it works best for phenomena such as mobility behaviour, air pollution, and other human activities that tend to become more dense near city centres, making it poorly suited to the analysis of geographic systems that lack a clearly defined central areas such as rural areas, polycentric conurbations, and countries.
ClockBoard is therefore useful only for a rather narrow set of urban analytics use cases: the analysis of geographic data in monocentric urban systems and inter-city comparison projects.

A broader set of limitations apply to *concentric segmented annuli for creating zoning systems in general*:
the approach, by definition, results in curved zones that do not follow local contours.
The division of London into ClockBoard zones, as illustrated in Figure \@ref(fig:london), creates zones covering both sides of the River Thames, for example.
Not only does this result in unusually shaped zones pieces and 'zone islands' separated from the rest of the zone of which they are part by physical barriers such as rivers and large roads, it also results in zones that may be more internally diverse than official zone systems which tend to (and are often designed to) include similar types of places into the same zone, with advantaged and disadvantages that depend on the application.
Zones based on concentric segmented annuli is that they have regular curved edges while in practice city borders are usually irregular.
Clipping the zones to official city boundaries can address this issue, but creates an additional problem: unhelpfully shaped and sized zones in the periphery of large cities.
For this reason, and to benefit from the standardised zones provided by ClockBoard, we recommend using the system without clipping in most cases: ignoring the historic boundary and defining the city bounds with reference to distance from the centre, as the ClockBoard system does can help think about urban systems in new ways and directly compare cities with different geographic extents according to their official boundaries, such as London and Paris.
A broader limitation of uniform grids that are not informed by local datasets, is that the zone shapes and sizes cannot respond to variables such as population density.
While this may be a major limitation for research projects involving only one city, it could be considered as an advantage when comparing multiple cities.
