...
In terms of the limitations of the *ClockBoard implementation of the concentric segmented annuli approach*, it is worth revisiting how the ClockBoard system was established.
We arrived at the ClockBoard zoning system after testing a range of alternative parameters that generated zones based on concentric segmented annuli and discussing the results informally with colleagues, including in a hackathon attended by around 20 self selected people.
That does not mean it is in any way 'optimal': the ClockBoard system described in this paper generated subjectively better results when the datasets we were using for urban analysis were visualised using ClockBoard zones than when using administrative zones or other zoning systems based on concentri segmented annuli.
...
A specific (and deliberate) limitation of the ClockBoard zoning system is that it works best for phenomena such as mobility behaviour, air pollution, and other human activities that become more dense, and therefore benefit from higher resolutions and smaller zones, near city centres.
...
A particular disadvantage of zones based on concentric segmented annuli is that they have regular curved edges while in practice city borders are usually irregular; this creates unhelpfully shaped and sized zones in the periphery of large cities, a limitation that can be overcome by ignoring the historic boundary and defining the city bounds with reference to distance from the centre.
A broader limitation of any automatically created zoning system that is spatially identical everywhere it is implemented, is that the zone shapes and sizes cannot respond to variables such as population density.
