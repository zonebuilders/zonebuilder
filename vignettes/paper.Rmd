...
abstract: |
  ...
  that enables comparison of geographic phenomena across different cities with
  zones of consistent and visually appealing sizes and shapes.
...
system("latexdiff vignettes.zonebuilder-paper.tex zonebuilder-paper.tex > diff.tex")
...
Our aim in this paper is not to tackle the MAUP directly.
The aim is to provide a 'ready made' zoning system that can demonstrate and to some extent control for (between cities) the effects of the MAUP by providing another way to aggregate and present data.
...
Functional zone classification is common in the field of remote sensing and associated sub-fields involved in analyzing and classifying raster datasets [@ciglic_evaluating_2019; @hesselbarth_landscapemetrics_2019].
...
The visualization of direction has been driven by new navigational requirements and datasets, with circular compasses and displays common in land and sea navigational systems since the mid 1900s [@honick_pictorial_1967]. Circular visualization techniques, in the form of rose diagrams, were used in a more recent study to indicate the most common road directions relative to North [@boeing_spatial_2021]. The resulting visualizations are attractive and easy to interpret, but are not geographical, in the sense that they cannot meaningfully be overlaid on mapped data. 
...
The specific motivations for embarking on the idea, and its implementation in open source software, in descending order of importance, were as follows:
...
-   Locating places and spatial entities in cities.
...
Of the four motivating problems and use cases outlined above the first --- comparing cities --- is likely to be the most important for many readers, especially from the perspective or urban data analysis and visualisation.
...
- **Concentric rings** --- formally called 'concentric annuli' --- which emphasize central locations and have been used to explore the relationships between the characteristics of 'focal trees' and surrounding trees in ecological research [@wills_persistence_2016], as shown in Figure \@ref(fig:options) (A).
...
# tmap_mode("view") # interactive visualization mode
...
# zip the file
zip(zipfile = "clockboard-complete.zip", flags = "-r", files = "clockboard-paper-resubmission-final-corrections")
piggyback::pb_upload("clockboard-complete.zip")
piggyback::pb_download_url("clockboard-complete.zip")
# https://github.com/zonebuilders/zonebuilder/releases/download/v0.0.2.9000/clockboard-complete.zip
```
...
appropriate output geography" [@martin_optimizing_1998], we started from scratch focusing on the key spatial attributes of distance and and bearing from the center.
...
<!-- Based on these criteria we developed the ClockBoard zoning system, which consists of regularly spaced rings forming 'donuts' (technically annuli) emanating from the city center and labelled A, B, C etc. -->
...
In terms of the limitations of the *ClockBoard implementation of the concentric segmented annuli approach*, it is associated with a fairly wide range of zone sizes and shapes, with zone areas ranging from 2 km^2^ in for zones in doughnut B to 33 km^2^ on the outermost doughnut E in a ClockBoard system with 5 rings (and a radius of 18 km).
...
A broader point is that too much academic research focuses only on a single city, without going to the effort of generalizing the findings to multiple cities [@alidadi_beyond_2018; @chandra_multi-objective_2021].
