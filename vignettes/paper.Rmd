...
(Not all cities have a 'monocentric' structure, something we discuss in Section \@ref(discussion-and-conclusion).)
...
Perhaps the biggest limitation of the system is its implicit assumption that cities are monocentric entities in which urban activity declines gradually with distance from the city centre.
...
This raises the question: how to design zoning systems for polycentric regions, or large regions that contain multiple monocentric settlements [@chandra_multi-objective_2021]?
...
A question for further research is how to fit one or several ClockBoards to a dense urban area consisting of several cities, of which none is dominating. For instance, the four major cities in the Netherlands (Amsterdam, Rotterdam, The Hague and Utrecht) are small and located about 40 kilometers from each other, with even smaller cities and towns in between. In this example, there is no dominant "gravitational force" to construct one ClockBoard around. Probably, the ClockBoard is less useful is there situations. However, ClockBoards could be constructed for each of these four cities, but should ideally be fit on each city alone including neighbouring towns.

While the utility of the zoning system is likely to be limited in many settings by these limitations, we believe that there are settings in which ClockBoard could provide substantial benefits, as demonstrated in three example applications.
...
We hope that the concept of the ClockBoard zoning system presented in this paper, and the ease with which open access data representing 'ClockBoards' for different cities can be created, will encourage more quantitative urban analytical research comparing different cities.
